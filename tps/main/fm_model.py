...
def model(t, y, p, *args):
    ...
    #if 'inject' in p.__dict__.keys():
    #    dict_ = p.inject
    #    for key in dict_:
    #        val_ = dict_[key]
    #        injection = 1 - (1/(1+exp(100*(t-val_[0]))) +
    #                          1/(1+exp(-100*(t-val_[1]))))
    #        if key == 'Na':
    #            p.NaCb = p.NaCb + injection*val_[2]
    #        elif key == 'K':
    #            p.KCb = p.KCb + injection*val_[2]
    #        elif key == 'Cl':
    #            p.ClCb = p.ClCb + injection*val_[2]
    #        elif key == 'Ca':
    #            p.CaCb = p.CaCb + injection*val_[2]
    #        elif key == 'Glu':
    #            p.GluCb = p.GluCb + injection*val_[2]
    # Voltages
    ...
    if 'block' in p.__dict__.keys():
        ...
        for key in dict_:
            ...
            if key == 'INaG':
                ...
                JEAATg = JEAATg*partialLeakblock
            elif key == 'INCXg':
